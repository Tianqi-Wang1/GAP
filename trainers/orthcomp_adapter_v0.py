...
def build_residual_dictionary_from_prototypes(
    class_prototypes,
    base_text_features,
    ...
    Args:
        class_prototypes:   [C, D], visual class prototypes
        base_text_features: [C, D], CLIP text classifiers
        normalize_inputs: whether to normalize p_c and t_c before subtraction
        normalize_residual: whether to normalize residual atoms
        tangent: whether to remove the component along t_c

    Returns:
        residual_basis: [C, D]
    """
    P = class_prototypes.float()
    T = base_text_features.float().to(P.device)
    ...
    if tangent:
        # Remove the component along each corresponding text classifier.
        T_hat = F.normalize(T, dim=-1)
        D = D - (D * T_hat).sum(dim=-1, keepdim=True) * T_hat
...
class NonlinearOrthCompProjLearner(nn.Module):
    ...
    def __init__(self, cfg, base_text_features, B_T, B_ref=None, B_con=None, B_R=None, G_init=None):
        ...
        # ---------- Linear path: same role as original G ----------
        self.linear = nn.Linear(r, D, bias=False)

        if G_init is not None:
            # G_init: [r, D]
            with torch.no_grad():
                self.linear.weight.copy_(G_init.float().t())  # Linear weight: [D, r]
        else:
            nn.init.zeros_(self.linear.weight)

        # ---------- Nonlinear residual path ----------
        # A conservative hidden dimension for few-shot setting.
        ...
        # Important: initialize nonlinear branch as zero.
        # So the model starts exactly from the linear OrthComp-Proj.
    ...
    def forward(self):
        T = self.base_text_features              # [K, D]
        _, R = self.compute_residual()           # [K, D]

        W = T + self.beta * R                    # [K, D]
        
        # return self.base_text_features + self.beta * self.text_feature_residuals
        return W
    
    
    
# class LowRankTextCondLearner(nn.Module):
#     """
#     Low-rank text-conditioned residual generator.
...
#     Key difference from OrthComp-Proj:
#         - no hard orthogonal projection
#         - A is a learnable shared residual basis, not class-specific weights
#         - H_theta generates class-wise coefficients from text condition
#     """

#     def __init__(self, cfg, base_text_features, B_T, B_ref=None, B_con=None):
#         super().__init__()
...
#         # -------- choose condition basis --------
#         try:
#             cond_basis = cfg.TRAINER.OrthComp_adapter.LR_COND_BASIS
#         except Exception:
#             cond_basis = "bt"   # options: "bt", "ref", "con"

#         if cond_basis == "ref" and B_ref is not None:
#             B_cond = B_ref
#             print(">> LowRankTextCondLearner uses B_ref as condition basis")
#         elif cond_basis == "con" and B_con is not None:
#             B_cond = B_con
#             print(">> LowRankTextCondLearner uses B_con as condition basis")
#         else:
#             B_cond = B_T
#             print(">> LowRankTextCondLearner uses B_T as condition basis")

#         self.register_buffer("B_cond", B_cond.float())  # [D, r]
...
#         # -------- hyperparameters --------
#         try:
#             m = int(cfg.TRAINER.OrthComp_adapter.LR_RESIDUAL_RANK)
#         except Exception:
#             m = 64

#         try:
#             hidden_dim = int(cfg.TRAINER.OrthComp_adapter.LR_HIDDEN_DIM)
#         except Exception:
#             hidden_dim = min(D, max(64, 4 * r))

#         try:
#             self.use_gate = bool(cfg.TRAINER.OrthComp_adapter.LR_USE_GATE)
#         except Exception:
#             self.use_gate = True

#         try:
#             self.gate_rho = float(cfg.TRAINER.OrthComp_adapter.LR_GATE_RHO)
#         except Exception:
#             self.gate_rho = 0.5

#         try:
#             self.use_set_context = bool(cfg.TRAINER.OrthComp_adapter.LR_USE_SET_CONTEXT)
#         except Exception:
#             self.use_set_context = False

#         try:
#             self.use_tangent = bool(cfg.TRAINER.OrthComp_adapter.LR_USE_TANGENT)
#         except Exception:
#             self.use_tangent = False

#         try:
#             self.normalize_basis = bool(cfg.TRAINER.OrthComp_adapter.LR_NORMALIZE_A)
#         except Exception:
#             self.normalize_basis = True

#         in_dim = 3 * r if self.use_set_context else r

#         print(f">> LowRankTextCondLearner: r={r}, m={m}, hidden={hidden_dim}")
#         print(f">> use_gate={self.use_gate}, use_set_context={self.use_set_context}, use_tangent={self.use_tangent}")

#         # -------- coefficient generator H_theta --------
#         self.coeff_net = nn.Sequential(
#             nn.Linear(in_dim, hidden_dim, bias=True),
#             nn.ReLU(inplace=True),
#             nn.Linear(hidden_dim, m, bias=False),
#         )

#         # Start from zero residual, i.e., W = T at initialization.
#         nn.init.zeros_(self.coeff_net[-1].weight)

#         # -------- learnable shared residual basis A --------
#         # A: [D, m], shared across all classes.
#         self.res_basis = nn.Parameter(torch.empty(D, m))
#         nn.init.orthogonal_(self.res_basis)

#         # -------- optional text-conditioned gate --------
#         if self.use_gate:
...
#             U = torch.cat([U, U_mean, U - U_mean], dim=-1)

#         return U

#     def compute_residual(self):
#         T = self.base_text_features              # [K, D]
#         U = self._build_condition()              # [K, r] or [K, 3r]

#         H = self.coeff_net(U)                    # [K, m]

#         if self.normalize_basis:
#             A = F.normalize(self.res_basis, dim=0)   # normalize each basis vector
#         else:
#             A = self.res_basis
...
#         if self.use_tangent:
#             R = classwise_tangent_projection(R, T)

#         if self.use_gate:
#             gate = 1.0 + self.gate_rho * torch.tanh(self.gate_net(U))  # [K, 1]
#             R = gate * R
...
#     def basis_regularization(self):
#         """
#         Optional weak orthogonality regularization for A.
#         Only use this if you later want to add it explicitly into the loss.
#         """
#         A = F.normalize(self.res_basis, dim=0)
#         I = torch.eye(A.size(1), device=A.device, dtype=A.dtype)
#         return ((A.t() @ A - I) ** 2).mean()
    
    
class PrototypeBasisTextCondLearner(nn.Module):
    """
    Text-conditioned residual generator with visual prototype basis.

    T: [K, D] text classifiers
    B_cond: [D, r] condition basis
    P: [M, D] visual prototype basis
    ...
    def __init__(self, cfg, base_text_features, B_T, visual_basis, B_ref=None, B_con=None):
        super().__init__()
        ...
        # choose condition basis
        try:
            cond_basis = cfg.TRAINER.OrthComp_adapter.PB_COND_BASIS
        except Exception:
            cond_basis = "bt"

        if cond_basis == "ref" and B_ref is not None:
            B_cond = B_ref
            print(">> PrototypeBasis uses B_ref as condition basis")
        elif cond_basis == "con" and B_con is not None:
            B_cond = B_con
            print(">> PrototypeBasis uses B_con as condition basis")
        else:
            B_cond = B_T
            print(">> PrototypeBasis uses B_T as condition basis")

        self.register_buffer("B_cond", B_cond.float())

        # # visual prototype basis P: [M, D]
        # P = F.normalize(visual_basis.float(), dim=-1)
        # self.register_buffer("visual_basis", P)
        
        try:
            self.normalize_visual_basis = bool(cfg.TRAINER.OrthComp_adapter.PB_NORMALIZE_BASIS)
        except Exception:
            self.normalize_visual_basis = True

        P = visual_basis.float()

        if self.normalize_visual_basis:
            P = F.normalize(P, dim=-1)
        ...
        try:
            hidden_dim = int(cfg.TRAINER.OrthComp_adapter.PB_HIDDEN_DIM)
        except Exception:
            hidden_dim = min(D, max(128, 4 * r))

        try:
            self.use_tanh_coeff = bool(cfg.TRAINER.OrthComp_adapter.PB_USE_TANH_COEFF)
        except Exception:
            self.use_tanh_coeff = False

        try:
            self.coeff_scale = float(cfg.TRAINER.OrthComp_adapter.PB_COEFF_SCALE)
        except Exception:
            self.coeff_scale = 1.0

        try:
            self.use_gate = bool(cfg.TRAINER.OrthComp_adapter.PB_USE_GATE)
        except Exception:
            self.use_gate = False

        try:
            self.gate_rho = float(cfg.TRAINER.OrthComp_adapter.PB_GATE_RHO)
        except Exception:
            self.gate_rho = 0.5

        print(f">> PrototypeBasisTextCondLearner: K={K}, D={D}, M={M}, r={r}, hidden={hidden_dim}")
        ...
        # initialize from CLIP classifier: residual = 0
        nn.init.zeros_(self.coeff_net[-1].weight)
    ...
    def compute_residual(self):
        ...
        if self.use_tanh_coeff:
            H = self.coeff_scale * torch.tanh(H)

        R = H @ self.visual_basis          # [K, D]

        if self.use_gate:
            gate = 1.0 + self.gate_rho * torch.tanh(self.gate_net(U))
            R = gate * R
...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, clip_model, base_text_features, B_T, B_ref, B_con, B_R, G_init=None, visual_basis=None):
        super().__init__()
        ...
        # self.prompt_learner = LowRankTextCondLearner(
        #     cfg,
        #     base_text_features=base_text_features,
        #     B_T=B_T,
        #     B_ref=B_ref,
        ...
        # self.prompt_learner = PrototypeBasisTextCondLearner(
        #     cfg,
        #     base_text_features=base_text_features,
        #     B_T=B_T,
        #     B_ref=B_ref,
    ...
    def train(self, mode: bool = True):
        ...
        # self.text_encoder.eval()   # 如果有冻结文本编码器
        return self
    ...
    #     # ---- visual adapter branch ----
    #     image_features = image_features.float()                     # 用 fp32 更稳
    ...
    def forward(self, image, return_text=False):
        ...
        if return_text:
            base_text_features = self.prompt_learner.get_base_text_features()
            base_text_features = F.normalize(base_text_features.float(), dim=-1)
            return logits, text_features, base_text_features
...
class OrthComp_adapter(TrainerX):
    ...
    def build_model(self):
        ...
        if cfg.TRAINER.OrthComp_adapter.BUILD_FEAT_CACHE:
            ...
            print(">> Building sample-level visual basis from all cached frozen image features...")
            # self.train_visual_basis = F.normalize(feats.float(), dim=-1)
            self.train_visual_basis = feats.float()
            self.train_visual_basis_labels = labels.long()

            print(f">> Train sample visual basis shape: {self.train_visual_basis.shape}")
        ...
        if cfg.TRAINER.OrthComp_adapter.BUILD_SUBSPACE:
            ...
            if osp.exists(subspace_path) and not cfg.TRAINER.OrthComp_adapter.SUBSPACE_FORCE_REBUILD:
                ...
                B_T, text_singular_values, text_rank, B_ref_Bt = build_text_basis(base_text_features, rank=cfg.TRAINER.OrthComp_adapter.TEXT_BASIS_RANK,
                                                                        rank_ref=cfg.TRAINER.OrthComp_adapter.REF_BASIS_RANK)
        ...
        G_init = None
        try:
            pb_basis_type = cfg.TRAINER.OrthComp_adapter.PB_BASIS_TYPE
        except Exception:
            pb_basis_type = "prototype"

        if pb_basis_type == "sample":
            visual_basis = self.train_visual_basis if hasattr(self, "train_visual_basis") else None
            print(">> Use sample-level visual features as visual basis")

        elif pb_basis_type == "residual_proto":
            assert hasattr(self, "train_class_prototypes")
            visual_basis = build_residual_dictionary_from_prototypes(
                class_prototypes=self.train_class_prototypes,
                base_text_features=base_text_features,
                normalize_inputs=True,
                normalize_residual=cfg.TRAINER.OrthComp_adapter.PB_NORMALIZE_BASIS,
                tangent=False,
            )
            print(">> Use class-level residual dictionary as visual basis: D = P - T")

        elif pb_basis_type == "residual_proto_tangent":
            assert hasattr(self, "train_class_prototypes")
            visual_basis = build_residual_dictionary_from_prototypes(
                class_prototypes=self.train_class_prototypes,
                base_text_features=base_text_features,
            ...
            print(">> Use tangent residual dictionary as visual basis")

        elif pb_basis_type == "both":
            assert hasattr(self, "train_class_prototypes")
            assert hasattr(self, "train_visual_basis")
            visual_basis = torch.cat(
                [
                    F.normalize(self.train_class_prototypes.float(), dim=-1),
                    F.normalize(self.train_visual_basis.float(), dim=-1),
                ],
                dim=0,
            )
            print(">> Use both class prototypes and sample-level visual features as basis")

        else:
            visual_basis = self.train_class_prototypes if hasattr(self, "train_class_prototypes") else None
            print(">> Use class-level visual prototypes as visual basis")
    ...
    #             # loss_c_reg = self.cfg.TRAINER.OrthComp_adapter.LAMBDA_C * learner.C.pow(2).mean()
    ...
    def forward_backward(self, batch):
        ...
        if prec == "amp":
            with autocast():
                output, text_features, base_text_features = self.model(
                    image, return_text=True
                )

                loss_ce = F.cross_entropy(output, label)
                loss_proto = self.compute_proto_ce_loss(text_features)
